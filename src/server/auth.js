  ...
  try {
    ...
    const info = db.transaction(() => {
      ...
      if (role === 'student') {
        ...
        ).run(userId, details.rollNumber, details.programme || 'N/A', details.year || 1);
      } else if (role === 'instructor') {
        db.prepare(
          'INSERT INTO instructor (user_id, phone, address, department) VALUES (?, ?, ?, ?)'
        ).run(userId, details.phone || '', details.address || '', details.department || 'N/A');
      }
...
router.get('/me', (req, res) => {
  ...
  try {
    ...
    const user = db.prepare('SELECT name, email FROM user WHERE user_id = ?').get(decoded.userId);
    if (!user) {
      res.clearCookie('token');
